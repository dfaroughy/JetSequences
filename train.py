...
parser.add_argument("--comet_api_key", type=str)
...
parser.add_argument("--num_train_jets", "-n_train", type=int, default=10_000_000)
parser.add_argument("--num_val_jets", "-n_val", type=int, default=500_000)
...
train_dataset = JetSequenceDataset(filepath=f"{config.data_path}/train_100M_binned_{config.num_bins[0]-1}_{config.num_bins[1]-1}_{config.num_bins[2]-1}/train_{config.jet_type}_10M_binned.h5",
                                   num_jets=config.num_train_jets, 
...
checkpoint_every_epoch = L.callbacks.ModelCheckpoint(
    ...
    every_n_epochs=5,                # save after each epoch
